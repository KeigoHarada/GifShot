          ...
          DragGesture(minimumDistance: 0)
            .onChanged { value in
              if dragStart == nil {
                ...
                Log.overlay.info(
                  "drag start: \(NSStringFromPoint(NSPoint(x: value.startLocation.x, y: value.startLocation.y)))"
                )
              }
            ...
            .onEnded { _ in
              if let rect = selectionRect, rect.width > 2, rect.height > 2 {
                Log.overlay.info(
                  "drag end with rect: \(NSStringFromRect(NSRect(x: rect.origin.x, y: rect.origin.y, width: rect.width, height: rect.height)))"
                )
